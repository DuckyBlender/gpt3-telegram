...
# Save command - saves the conversation to a txt file and sends it to the user
...
def save(message):
    ...
    con = sqlite3.connect('users.db')
    cursor = con.cursor()
    cursor.execute("SELECT chat_messages FROM users WHERE user_id = ?", (user_id,))
...
def chat(message):
    ...
    bot.send_message(message.chat.id, f"```{convo}```\n`Bot:` *{response}*", parse_mode='Markdown')
